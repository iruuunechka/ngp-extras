 ...
 */
object IntOps {
  final val Numeric: Numeric[Int] = implicitly
  final val Ordering: Ordering[Int] = implicitly

  @inline
  final def mapSum(r: Range, fun: Int => Int): Int = {
    if (r.isEmpty) 0 else {
      val end = r.end
      val step = r.step
      var curr = r.start
  ...
  @inline
  final def mapProduct(r: Range, fun: Int => Int): Int = {
    if (r.isEmpty) 1 else {
      val end = r.end
      val step = r.step
      var curr = r.start
  ...
  @inline
  final def mapMin(range: Range, fun: Int => Int): Int = {
  ...
  @inline
  final def mapMax(range: Range, fun: Int => Int): Int = {
