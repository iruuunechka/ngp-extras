...
abstract class CodomainComparator[C: CodomainType] {
   ...
   */
  val codomainOrdering: Ordering[C] = (x: C, y: C) => apply(x, y)
   ...
   */
  def evaluatedOrdering[D: DomainType]: Ordering[Evaluated[D, C]] = (x: Evaluated[D, C],
                                                                     y: Evaluated[D, C]) => apply(x.output, y.output)
}
...
object CodomainComparator {
  ...
  class ID[C: CodomainType](cmp: (C, C) => Int) {
     ...
     */
    def increasing: CodomainComparator[C] = new CodomainComparator[C]() {
      def apply(lhs: C, rhs: C): Int = cmp(lhs, rhs)
    }
    /**
     * Returns a decreasing codomain comparator.
     * @return the decreasing codomain comparator.
     */
    def decreasing: CodomainComparator[C] = new CodomainComparator[C]() {
      def apply(lhs: C, rhs: C): Int = cmp(rhs, lhs)
    }
  }
   ...
   */
  def apply[C: CodomainType](): Detected[C] = new Detected
}
