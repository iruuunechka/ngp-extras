...
abstract class MultipleCriteria[C: CodomainType] {
  ...
  def projection(indices: Seq[Int]): MultipleCriteria[C] = {
    ...
    new MultipleCriteria[C] {
      def orderingForCriterion(criterion: Int): Ordering[C] = implOrderingForCriterion(criterion)
      def nameOfCriterion(criterion: Int): String = implNameOfCriterion(criterion)
...
object MultipleCriteria {
  ...
  def fromIndexedSeqWithElementOrdering[C, T](names: String*)(
    ...
    private val orderings = (0 until criteria) map { i =>
      new Ordering[C] {
        def compare(x: C, y: C): Int = ordering.compare(x(i), y(i))
      }
    }
    def nameOfCriterion(criterion: Int): String = myNames(criterion)
    def numberOfCriteria: Int = criteria
    def orderingForCriterion(criterion: Int): Ordering[C] = orderings(criterion)
  }
  ...
  def fromIndexedSeqOfNumeric[C, T](names: String*)(
    implicit codomain: CodomainType[C],
             c2indexedSeq: C => IndexedSeq[T],
             numeric: Numeric[T]
  ): MultipleCriteria[C] with CriterionToDouble[C] = {
    new MultipleCriteria[C] with CriterionToDouble[C] {
      private val criteria = names.size
      private val myNames = names.toIndexedSeq
      private val orderings = (0 until criteria) map { i =>
        new Ordering[C] {
          def compare(x: C, y: C): Int = numeric.compare(x(i), y(i))
        }
      }
      def numberOfCriteria: Int = criteria
      def nameOfCriterion(criterion: Int): String = myNames(criterion)
      def orderingForCriterion(criterion: Int): Ordering[C] = orderings(criterion)
      def criterionToDouble(codomain: C, criterion: Int): Double = numeric.toDouble(codomain(criterion))
    }
  }
   ...
   * @return the `MultipleCriteria` with `CriterionToDouble` object.
   */
  def fromExtractorOfNumeric[
    C: CodomainType,
    T: Numeric
  ](extractor: (C, Int) => T, names: String*): MultipleCriteria[C] with CriterionToDouble[C] = {
    new MultipleCriteria[C] with CriterionToDouble[C] {
      ...
      private val orderings = (0 until criteria) map { i =>
        new Ordering[C] {
          def compare(x: C, y: C): Int = num.compare(extractor(x, i), extractor(y, i))
        }
      }
      def numberOfCriteria: Int = criteria
      def nameOfCriterion(criterion: Int): String = myNames(criterion)
      def orderingForCriterion(criterion: Int): Ordering[C] = orderings(criterion)
      def criterionToDouble(codomain: C, criterion: Int): Double = num.toDouble(extractor(codomain, criterion))
    }
  }
}
