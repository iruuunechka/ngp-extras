...
import ru.ifmo.ctd.ngp.opt.multicriteria.{DominationComparisonResult, NonDominatedSorting, MultipleCriteria}
...
final class DebLinearMemory[D : DomainType, C : CodomainType : MultipleCriteria] extends NonDominatedSorting[D, C] {
  def apply(data: IndexedSeq[Evaluated[D, C]]): IndexedSeq[IndexedSeq[Evaluated[D, C]]] = {
    ...
    val arrays = IndexedSeq.tabulate(maxLayer + 1)(_ => IndexedSeq.newBuilder[Evaluated[D, C]])
    for (i <- 0 until sz) {
