...
abstract class Iteration[D: DomainType, C: CodomainType, W[+_D, +_C]: WorkingSetType]
   ...
   */
  final def apply(ws: W[D, C]): W[D, C] = {
...
object Iteration {
  class Detected[D: DomainType, C: CodomainType, W[+_D, +_C]: WorkingSetType] {
    ...
    def use(fun: W[D, C] => W[D, C]): Iteration[D, C, W] = new Iteration[D, C, W]() {
      protected def performIteration(ws: W[D, C]): W[D, C] = fun(ws)
    }
    /**
    ...
    def fromSelectionMutationEvaluateUpdate(
      implicit selection: Selection[D, C, W],
               ...
               update: Update[D, C, W]
    ): Iteration[D, C, W] = new Iteration[D, C, W] {
      protected def performIteration(ws: W[D, C]): W[D, C] = update(ws, evaluator(mutation(selection(ws))))
    }
    /**
      * Constructs an `Iteration` object from `Initialization`, `Selection`, `Mutation`, `Evaluator` and `Update` objects
      * available through implicits.
      *
      * It will restart whenever the given predicate evaluates to `true`.
      *
      * @param restartPredicate the predicate which, if true, restarts the algorithm
      * @param initialization the `Initialization` object.
      * @param selection the `Selection` object.
      * @param mutation the `Mutation` object.
      * @param evaluator the `Evaluator` object.
      * @param update the `Update` object.
      * @return
      */
    def fromInitializationSelectionMutationEvaluateUpdateWithRestartWhen(restartPredicate: => Boolean)(
      implicit initialization: Initialization[D, C, W],
               selection: Selection[D, C, W],
               mutation: Mutation[D, C],
               evaluator: Evaluator[D, C],
               update: Update[D, C, W]
    ): Iteration[D, C, W] = new Iteration[D, C, W] {
      protected def performIteration(ws: W[D, C]): W[D, C] = {
        if (restartPredicate) initialization() else update(ws, evaluator(mutation(selection(ws))))
      }
    }
  }
