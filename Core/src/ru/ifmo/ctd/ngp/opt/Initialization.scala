...
abstract class Initialization[D: DomainType, C: CodomainType, W[+_D, +_C]: WorkingSetType]
   ...
   */
  final def apply(): W[D, C] = {
    initializationStartedAccessor.fire(())
    performInitialization |> initializationFinishedAccessor.fire
  }
  /**
   * Performs an initialization of an optimization algorithm.
   * @return the working set after the initialization.
...
object Initialization {
  class Detected[D: DomainType, C: CodomainType, W[+_D, +_C]: WorkingSetType] {
    ...
    ): Initialization[D, C, W] = new Initialization[D, C, W]() {
      protected def performInitialization(): W[D, C] = indexedSeq2WorkingSet(evaluator(it.toIndexedSeq))
    }
    /**
    ...
    def fromEvaluated(it: Iterable[Evaluated[D, C]])(
      ...
      protected def performInitialization(): W[D, C] = indexedSeq2WorkingSet(it.toIndexedSeq)
    }
    /**
    ...
    def use(fun: => W[D, C]): Initialization[D, C, W] = new Initialization[D, C, W]() {
      protected def performInitialization(): W[D, C] = fun
    }
    /**
    ...
    def useDomainGenerator(fun: => D, count: Int)(
      ...
      protected def performInitialization(): W[D, C] = indexedSeq2WorkingSet(evaluator(IndexedSeq.fill(count)(fun)))
    }
    /**
    ...
    def useElementGenerator[T](fun: => T, size: Int, count: Int)(
      ...
      protected def performInitialization(): W[D, C] = indexedSeq2WorkingSet(evaluator(IndexedSeq.fill(count) {
