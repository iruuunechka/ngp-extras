...
import ru.ifmo.ctd.ngp.opt.{Evaluated, RandomSource, SequenceExecutor}
...
object Mutation {
  object Standard {
    object BitSet {
      def singlePointMutation(n: Int)(implicit random: RandomSource): ScalaBitSet => ScalaBitSet = { x =>
        val idx = random().nextInt(n)
        if (x(idx)) x - idx else x + idx
      }
      def independentPointMutation(n: Int)(implicit random: RandomSource): ScalaBitSet => ScalaBitSet = { x =>
        val mbs = ScalaBitSet.newBuilder
        val rng = random()
        for (i <- 0 until n) {
          if (rng.nextInt(n) == 0) {
            if (!x(i)) mbs += i
      ...
      def independentPointMutation(n: Int, probability: Double)(implicit random: RandomSource): ScalaBitSet => ScalaBitSet = { x =>
        val mbs = ScalaBitSet.newBuilder
        val rng = random()
        for (i <- 0 until n) {
  ...
  class Detected[D: DomainType, C: CodomainType] {
    def usingEvaluatedToDomains(function: IndexedSeq[Evaluated[D, C]] => IndexedSeq[D]): Mutation[D, C] = {
      new Mutation[D, C] {
        def apply(evaluated: IndexedSeq[Evaluated[D, C]]): IndexedSeq[D] = function(evaluated)
      }
    }
    def usingDomainsToDomains(function: IndexedSeq[D] => IndexedSeq[D]): Mutation[D, C] = {
      new Mutation[D, C] {
        def apply(evaluated: IndexedSeq[Evaluated[D, C]]): IndexedSeq[D] = {
          function(evaluated.map(_.input))
        }
      }
    }
    def using(function: D => D)(implicit seq: SequenceExecutor): Mutation[D, C] = {
      new Mutation[D, C] {
        def apply(evaluated: IndexedSeq[Evaluated[D, C]]): IndexedSeq[D] = {
          seq.map[Evaluated[D, C], D](evaluated, t => function(t.input))
        }
      }
    }
    def using(function: D => D, times: => Int)(implicit seq: SequenceExecutor): Mutation[D, C] = {
      new Mutation[D, C] {
        def apply(evaluated: IndexedSeq[Evaluated[D, C]]): IndexedSeq[D] = {
          seq.mapN[Evaluated[D, C], D](evaluated, t => function(t.input), times)
        }
      }
    }
