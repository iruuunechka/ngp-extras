...
import ru.ifmo.ctd.ngp.opt.{CodomainComparator, Evaluated, RandomSource}
...
object Selection {
  class Detected[D: DomainType, C: CodomainType, W[+_D, +_C]: WorkingSetType] {
     ...
     */
    def using(function: W[D, C] => IndexedSeq[Evaluated[D, C]]): Selection[D, C, W] = new Selection[D, C, W] {
      override def apply(workingSet: W[D, C]): IndexedSeq[Evaluated[D, C]] = function(workingSet)
    }
    /**
    ...
    ): Selection[D, C, W] = new Selection[D, C, W] {
      override def apply(workingSet: W[D, C]): IndexedSeq[Evaluated[D, C]] = {
        workingSet2TraversableOnce(workingSet).toIndexedSeq
      }
    }
    /**
     * Returns a `Selection` object that returns all `Evaluated` objects in the working set,
