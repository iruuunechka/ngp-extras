...
trait InitializationStartedEvent {
  ...
  val initializationStartedEvent: EventBackend[Unit] = initializationStartedAccessor.event
}

object InitializationStartedEvent {
  def apply()(implicit event: InitializationStartedEvent): EventBackend[Unit] = {
    event.initializationStartedEvent
  }
}
