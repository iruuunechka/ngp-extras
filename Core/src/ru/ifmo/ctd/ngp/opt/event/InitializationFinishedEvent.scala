...
trait InitializationFinishedEvent[W] {
  ...
  val initializationFinishedEvent: EventBackend[W] = initializationFinishedAccessor.event
}

object InitializationFinishedEvent {
  def apply[W]()(implicit event: InitializationFinishedEvent[W]): EventBackend[W] = {
    event.initializationFinishedEvent
  }
}
