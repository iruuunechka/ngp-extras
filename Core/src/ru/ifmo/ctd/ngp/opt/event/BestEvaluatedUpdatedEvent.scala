...
trait BestEvaluatedUpdatedEvent[D, C] {
  ...
  val bestEvaluatedUpdatedEvent: EventBackend[Evaluated[D, C]] = bestEvaluatedUpdatedAccessor.event
}

object BestEvaluatedUpdatedEvent {
  def apply[D, C]()(implicit event: BestEvaluatedUpdatedEvent[D, C]): EventBackend[Evaluated[D, C]] = {
    event.bestEvaluatedUpdatedEvent
  }
}
