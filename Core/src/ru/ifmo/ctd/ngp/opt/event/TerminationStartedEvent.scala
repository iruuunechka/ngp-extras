...
trait TerminationStartedEvent {
  ...
  val terminationStartedEvent: EventBackend[Termination.Reason] = terminationStartedAccessor.event
}

object TerminationStartedEvent {
  def apply()(implicit event: TerminationStartedEvent): EventBackend[Termination.Reason] = {
    event.terminationStartedEvent
  }
}
