...
trait IterationFinishedEvent[W] {
  ...
  val iterationFinishedEvent: EventBackend[W] = iterationFinishedAccessor.event
}

object IterationFinishedEvent {
  def apply[W]()(implicit event: IterationFinishedEvent[W]): EventBackend[W] = {
    event.iterationFinishedEvent
  }
}
