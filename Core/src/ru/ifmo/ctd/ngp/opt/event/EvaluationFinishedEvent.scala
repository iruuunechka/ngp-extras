...
trait EvaluationFinishedEvent[D, C] {
  ...
  val evaluationFinishedEvent: EventBackend[IndexedSeq[Evaluated[D, C]]] = evaluationFinishedAccessor.event
}

object EvaluationFinishedEvent {
  def apply[D, C]()(implicit event: EvaluationFinishedEvent[D, C]): EventBackend[IndexedSeq[Evaluated[D, C]]] = {
    event.evaluationFinishedEvent
  }
}
