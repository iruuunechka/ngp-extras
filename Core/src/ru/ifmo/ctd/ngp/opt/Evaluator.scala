...
abstract class Evaluator[D: DomainType, C: CodomainType] extends EvaluationFinishedEvent[D, C] {
   ...
   */
  final def apply(inputs: IndexedSeq[D]): IndexedSeq[Evaluated[D, C]] = {
    inputs |> evaluate |> evaluationFinishedAccessor.fire
  }
  /**
   * Computes the output values (from the codomain) for the given inputs (from the domain)
   * and wraps them into the specified `Evaluated` object type.
...
object Evaluator {
  class Detected[D: DomainType, C: CodomainType] {
     ...
     */
    def usingFunction(function: D => C)(implicit sequenceExecutor: SequenceExecutor): Evaluator[D, C] = {
      new Evaluator[D, C]() {
        def evaluate(inputs: IndexedSeq[D]): IndexedSeq[Evaluated[D, C]] = {
          sequenceExecutor.map(inputs, (d: D) => Evaluated(d, function(d)))
        }
      }
    }
  }
