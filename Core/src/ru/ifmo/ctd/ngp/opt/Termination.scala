...
abstract class Termination[D: DomainType, C: CodomainType, W[+_D, +_C]: WorkingSetType]
   ...
   */
  final def apply(ws: W[D, C]): Option[Termination.Reason] = terminationReason(ws) match {
...
object Termination {
  ...
  case class Pluggable[D: DomainType, C: CodomainType, W[+_D, +_C]: WorkingSetType]() extends Termination[D, C, W] {
    ...
    protected def terminationReason(ws: W[D, C]): Option[Reason] = plugins.view.flatMap(_(ws)).headOption
  }
}
