...
import ru.ifmo.ctd.ngp.demo.ffchooser.multicriteria.temp.MultiOAEnvironment;
 ...
 */
public class TSPMulticriteriaRunner {
...
//        System.out.println("------");
    ...
    private static void runTsp(TSPProblem problem, int populationSize, int stepsCount, String instance, int runs) throws FileNotFoundException {
        ...
        for (int i = 0; i < runs; i++) {
            ...
            MulticriteriaAlgorithm<List<Integer>> algo =  new TSPNSGA2Multicriteria(targetFitness, evaluators, factory, mutation, crossover, crossoverProbability, populationSize, rand, problem);

            algo.addPrinter(new CompactPrinter<>());

            MultiStateCalculator<String, Integer> state = new MultiTripleState(generations + 1, problem.getMax());
            MultiRewardCalculator reward = new MultiFixedBestTargetReward(-1, 0, 1);
            //new FixedBestReward(0, 0.5, 1);
            MultiOAEnvironment env = new MultiOAEnvironment(algo, reward, state);

            Agent<String, Integer> agent = new //EGreedyAgent<>(0.01, 1.0, 0.6, 0.1);
                    EpsQRLCDTSP<>(0.6, 0.1, 0.0, 0.001, new FreezeStrategy(0.95, 40, 0), problem);
...
//                pw = new PrintWriter("../TSP/" + "knowles" + instance + "GA" + state.getName() + agent.toString());
                pw = new PrintWriter("../TSP/multi/" + instance + "EA" + state.getName() + agent.toString()+"test");
            ...
            if ((int) (problem.getMax() - env.getFinalBestTargetValue()) <= (int) problem.solution) {
                counter++;
            }
