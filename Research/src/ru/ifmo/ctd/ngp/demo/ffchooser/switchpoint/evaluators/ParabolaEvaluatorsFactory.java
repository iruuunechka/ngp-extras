...
/**
 * Factory of <code>{(x - p)^2, (length - p)^2 - (x - p)^2, [x / k]}</code> evaluators list.
 * The target fitness evaluator is <code>[x / k]</code>.
 * 
 * @author Arina Buzdalova
 */
public class ParabolaEvaluatorsFactory implements EvaluatorsFactory {
	private static final long serialVersionUID = 8305815633033588051L;
    ...
    /**
	 * Creates {@link ParabolaEvaluatorsFactory} with the specified parameters
	 ...
	 */
	public ParabolaEvaluatorsFactory(@ParamDef(name = "divisor") int divisor, @ParamDef(name = "length") int length, @ParamDef(name = "switchPoint") int switchPoint) {
	...
	public List<FunctionalFitness> getEvaluators() {
		...
		evaluators.add(new FunctionalFitness(new SecondPower(c + 1, -1, -switchPoint)));
		evaluators.add(new FunctionalFitness(new SecondPower(0, 1, -switchPoint)));
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		ParabolaEvaluatorsFactory other = (ParabolaEvaluatorsFactory) obj;
