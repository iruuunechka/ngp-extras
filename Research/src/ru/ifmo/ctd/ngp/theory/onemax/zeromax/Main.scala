...
/**
 * Main for OneMax+ZeroMax controlled by (epsilon)-greedy RL algorithm.
 *
...
object Main extends App {
  ...
  val runs = 100
  val rootDir = "onemax-zeromax-logs"
  ...
  class Task(namePrefix: String, val onePlusOne: Boolean, val states: StateInfo, fun: Int => Double) {
    def run(n: Int) = fun(n)
    def name = s"$namePrefix-${if (onePlusOne) "one" else "rls"}-$states"
  ...
  println("OneMax + ZeroMax")
  
  val (one, rmhc) = allTasks partition (_.onePlusOne)
  for ((tasks, title) <- Seq((rmhc, "RLS"), (one, "(1+1)"))) {
    println(title)
    for (n <- 2 to 99) {
